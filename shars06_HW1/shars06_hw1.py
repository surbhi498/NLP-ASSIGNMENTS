...
Original file is located at
    https://colab.research.google.com/drive/1RQPuXrrhN_jmOVFqFxwmcSTi6hLsXcwO
"""
...
def replace_all(text, mydict):
    for gb, us in mydict.items():
      ...
      string = re.sub(f'(?<![a-zA-Z]){gb}(?![a-z-Z])', us, text)
    return text



def process_regex(mg):
    ...
    f = open('regex.txt', 'w')
    with open(mg,'r') as file:

    # reading each line    
       for line in file:
        us_text = replace_all(line, us2gb) 
        f.write(us_text)

process_regex('/content/Plain.txt')

import re
def replace_all(text, mydict):
    for gb, us in mydict.items():
      #text = text.replace(us, gb)
       text = re.sub(f'(?<![a-zA-Z]){gb}(?![a-z-Z])', us, text)
 
    return text

def process_regex1(fg):
   jk = {'Dr.' :'Doctor', 
      'Mr.' : 'Mister', 
      'Ms' : 'Miss', 
      'Mrs.' : 'Misses'}
   f = open('regexo.txt', 'w')
   with open(fg,'r') as file:

    # reading each line    
       for line in file:
          #for word in line.split():
            us_text = replace_all(line, jk) 
            f.write(us_text)

process_regex1('/content/regex.txt')